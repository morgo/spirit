...
func printDefinitions(formatter Formatter, definitions []*parser.Definition) error {
...
func handleStart(f Formatter, definition *parser.Definition) error {
...
func handleUnion(f Formatter, definition *parser.Definition) error {
	if err := Ensure(definition).
...
func handleProlog(f Formatter, definition *parser.Definition) error {
	if err := Ensure(definition).
		and(definition.Value).NotNil(); err != nil {
		return err
	}
...
func handleReservedWordTagNameList(f Formatter, def *parser.Definition) error {
	if err := Ensure(def).
...
func printNameListVertical(f Formatter, names NameArr) (err error) {
	rest := names
	for len(rest) != 0 {
...
func printSingleName(f Formatter, name *parser.Name, maxCharLength int) error {
...
func printRules(f Formatter, rules RuleArr) (err error) {
...
func printRuleBody(f Formatter, rule *parser.Rule) error {
...
func handleAction(f Formatter, rule *parser.Rule, action *parser.Action, isFirstItem bool) error {
...
func handlePrecedence(f Formatter, p *parser.Precedence, isFirstItem bool) error {
	if p == nil {
		return nil
	}
...
func checkInconsistencyInYaccParser(f Formatter, rule *parser.Rule, counter int) error {
