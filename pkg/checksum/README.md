...
- **Scaling** is opt-in via `AutoscaleConfig`, and adjusts the live worker count during a pass. Two signals drive it:
  ...
  - The **change-feed backlog**, which works everywhere. The feed flushes concurrently with the checksum, and its backlog gates cut-over — if it grows unboundedly the binlogs may be purged before a resume can replay them. If the feed is losing ground, the checksum's reads are winning a race against writes that have to finish, so a worker is shed. On stock MySQL this is the only shedding lever, and recovery is capped at the configured concurrency.
    ...
    What counts as "losing ground" is specifically a rising **post-flush residual**, not a rising backlog. The pending count is a sawtooth: it climbs on every sample between flushes and drops when one lands, so at the production numbers (5s control tick, 30s flush interval) the rising edge alone is six samples long and its slope says nothing about whether the feed is coping. What a healthy feed guarantees is that each flush drains it to near zero. So the controller tracks the *trough* of a window spanning two flush intervals and compares successive troughs: near-zero troughs mean the feed is keeping up however heavy the write load, while a trough that is large and climbing means work is surviving flushes and accumulating. A verdict holds for a whole window and suppresses growth as well as shedding — one-shot shedding alone would just be cancelled out by the next growth tick.
