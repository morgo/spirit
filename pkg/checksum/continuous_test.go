...
func TestContinuousFactoryDiscardsResumeEvidence(t *testing.T) {
	for _, mode := range []string{"single", "distributed", "lockless"} {
		t.Run(mode, func(t *testing.T) {
			...
			ctx, cancel := context.WithCancel(t.Context())
			cancel()
			require.NoError(t, checker.(ContinuousChecker).RunContinuous(ctx))
...
func TestLocklessContinuousReusesCheckerAfterInitialPass(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		go func() { done <- checker.(ContinuousChecker).RunContinuous(ctx) }()
		synctest.Wait()
		require.False(t, checker.(ContinuousChecker).ContinuousActive(), "initial interval is idle")
		<-chunker.entered
		require.True(t, checker.(ContinuousChecker).ContinuousActive(), "scanning is active")
		...
		require.False(t, checker.(ContinuousChecker).ContinuousActive(), "inter-pass pacing is idle")
		cancel()
		require.NoError(t, <-done)
		require.False(t, checker.(ContinuousChecker).ContinuousActive(), "finished checker is idle")
		require.Equal(t, feed.starts, feed.stops)
	})
}
