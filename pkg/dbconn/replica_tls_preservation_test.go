...
func TestTLSParameterPreservation(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "tls=required should be preserved",
			mainTLSMode:    "PREFERRED",
			replicaDSN:     "user:pass@tcp(replica:3306)/db?tls=required",
			expectedDSN:    "user:pass@tcp(replica:3306)/db?tls=required",
			shouldPreserve: true,
			description:    "Explicit tls=required should override main PREFERRED mode",
		},
		{
		...
		{
			name:           "tls=verify-identity should be preserved",
			mainTLSMode:    "PREFERRED",
			replicaDSN:     "user:pass@tcp(replica:3306)/db?tls=verify-identity",
			expectedDSN:    "user:pass@tcp(replica:3306)/db?tls=verify-identity",
			shouldPreserve: true,
			description:    "Explicit tls=verify-identity should override main PREFERRED mode",
		},
		{
...
func TestMixedEnvironmentTLSScenarios(t *testing.T) {
	scenarios := []struct {
		...
		},
		{
			...
			replicaDSN:  "user:pass@tcp(on-prem-replica.corp.com:3306)/db?tls=verify-identity",
			expectedTLS: "tls=verify-identity",
			expectError: false,
