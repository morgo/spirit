...
- `%r` verb: splices a string argument into the SQL verbatim, with no quoting
  and no format interpretation. Use it to embed raw user-provided SQL (such as
  an ALTER clause) into a trusted format string. Never concatenate user SQL
  into the format string itself: `%n` / `%?` / `%%` sequences inside it (e.g.
