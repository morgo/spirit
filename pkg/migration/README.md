...
The common **data lock** concern is the source table. By default Spirit uses the buffered copier, which reads rows into Spirit and writes them through the applier (`REPLACE INTO _new VALUES (...)`) instead of running `INSERT ... SELECT`, so it takes no locks on the source. The change source is likewise always buffered — it applies binlog row images and never runs `SELECT FROM original` — so the default path eliminates source-side data lock contention end-to-end. See [pkg/change/README.md](../change/README.md) for the subscription design.

The legacy `--unbuffered` copier instead issues `INSERT IGNORE INTO _new ... SELECT FROM original`, and the `SELECT` side of `INSERT ... SELECT` takes shared row locks rather than using MVCC. Production workloads touching the same rows can contend with those locks. When using `--unbuffered`, the main knob to mitigate this is `target-chunk-time`: smaller chunks mean each copy statement holds locks for less time.
