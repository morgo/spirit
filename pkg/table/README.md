...
- With the legacy unbuffered copier, source row locks are held for the duration of a chunk copy (`INSERT .. SELECT`), so a large chunk can block other operations on the table. The default buffered copier reads with MVCC and takes no source row locks, but a large chunk still buffers more rows in memory.
...
```
- The rows between the last chunk pointer and the new value are then handed to the copier (the chunker only computes the boundaries). The default buffered copier reads that range with a `SELECT` and writes it via the applier (`REPLACE INTO _new`); the legacy unbuffered copier issues `INSERT IGNORE .. SELECT` directly.
