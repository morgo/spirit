...
- The copier reads with MVCC and takes no source row locks, but a large chunk buffers more rows in memory.

All chunkers support "dynamic chunking," which means that rather than a fixed chunk size, the chunker adjusts the chunk size to meet a target. This tends to be a better approach than specifying a fixed chunk size, because the chunk size can vary wildly depending on the table. There are two target signals, selected via `ChunkerConfig`:

- **Time** (`TargetChunkTime`, defaulting to `ChunkerDefaultTarget` = `5s`): the servo aims for a wall-clock time per chunk. This is the checksum's signal: its chunk time is a faithful function of chunk size. (When the copier also sized by time, we typically saw the chunk size reduce significantly as the new table got larger, compensating for larger insert times — more so on Aurora than MySQL, because on IO-bound workloads Aurora does not have the [change buffer](https://dev.mysql.com/doc/refman/8.0/en/innodb-change-buffer.html).)
- **Memory** (`TargetChunkBytes`, the copier's default, exposed as the `--target-chunk-size` flag): the servo aims for an in-memory byte budget per chunk, using the size of the rows the copier reads into memory. The copier's measured chunk time includes waiting behind the write queue, which inflates under load independently of chunk size — so the time signal would collapse the chunk size to the row floor under backpressure. Bytes/row is a stable property of the data, so the byte signal stays convergent and keeps chunks large enough to engage read-ahead. The servo math (p90 of the last 10 chunks, 1.5x-per-step growth cap, `100,000`-row ceiling, `10`-row floor, 5x panic-shrink) is identical for both signals.

Spirit should be aggressive in copying, but there should only be minimal elevation in p99 response times. The row-lock contention that originally motivated the time target came from the legacy unbuffered copier's `INSERT .. SELECT` (since removed — the copier reads with MVCC and does not lock source rows), so the time signal now only paces the checksum, whose per-chunk work is a server-side CRC and returns a single row. `ChunkerDefaultTarget` is `5s`: it is a constant rather than a flag, because a *copy* knob was what users actually wanted to tune and that is `--target-chunk-size`. `5s` is also the ceiling — it is unlikely that users can tolerate larger than a 5s latency hit for a single query on an OLTP system, and since we adjust various lock wait timeouts based on the assumption that chunks are about this size, increasing beyond `5s` would require additional tuning.
...
```
- The rows between the last chunk pointer and the new value are then handed to the copier (the chunker only computes the boundaries). The copier reads that range with a `SELECT` and writes it via the applier (`REPLACE INTO _new`).

The composite chunker is very good at dividing the chunks up equally, since barring a brief race condition each chunk will match exactly the `chunkSize` value. The main downside is that it becomes a little bit wasteful when you have `AUTO_INCREMENT` `PRIMARY KEY`s and rarely delete data. In this case, you waste the initial `SELECT` statement, since the client could easily calculate the next chunk pointer by adding `chunkSize` to the previous chunk pointer. A second issue is that the `KeyAboveHighWatermark` optimization is more complex for the composite chunker than for the optimistic chunker. It works correctly for numeric, binary, and temporal primary key types, but for `VARCHAR`/`TEXT` columns with collations, Go's byte-order comparison may differ from MySQL's collation order (e.g., `'aa' = 'AA'` in `utf8mb4_0900_ai_ci`). Any discrepancies are caught by the checksum phase, since watermark optimizations are disabled before checksumming begins (see [issue #479](https://github.com/block/spirit/issues/479)).
