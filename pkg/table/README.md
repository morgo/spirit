...
- **Time** (`TargetChunkTime`, e.g. `500ms`): the servo aims for a wall-clock time per chunk. This is the checksum's signal: its chunk time is a faithful function of chunk size. (When the copier also sized by time, we typically saw the chunk size reduce significantly as the new table got larger, compensating for larger insert times — more so on Aurora than MySQL, because on IO-bound workloads Aurora does not have the [change buffer](https://dev.mysql.com/doc/refman/8.0/en/innodb-change-buffer.html).)
