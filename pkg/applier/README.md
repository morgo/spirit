...
`Stats()` carries all of it — and the metrics sink emits all of it — but `Stats.String()` renders only a subset onto the runner status line, since that line is read by a human every 30 seconds ([#329](https://github.com/block/spirit/issues/329)). Always shown: `applier-queue`, `applier-workers`, `applier-queue-wait-p50`, `applier-write-p50`, `applier-write-p90`. Shown only when they carry a diagnosis: `applier-build-p50` once build is ≥25% of write (client-CPU bound), and `applier-handoff-p50` once handoff reaches 1ms (blocked behind the completion path). Both are silent on a healthy run, so their *presence* is the signal — read their absence as "not the problem", not as a missing field.
