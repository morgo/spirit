...
One bound to respect if these are retuned: chunklets have to keep the write pool fed, so `chunks-in-flight × chunklets-per-chunk` must stay above the write worker count. At a 16 MiB target chunk, ~22 chunks in flight and a write ceiling of 188, that requires ≥ ~8.5 chunklets per chunk — the byte budget can only roughly *double* before the largest pools begin starving. (When the row cap binds instead, chunklets-per-chunk is set by the chunk's row count and the bound is much looser.)
