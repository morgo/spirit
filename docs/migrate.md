...
- **Growth** happens only with the flag *and* only on Aurora, since it uses the same utilization band as the copy phase. On stock MySQL the flag does not engage at all, so the checksum can shed and then recover, but never exceed, the configured [threads](#threads).
...
One structural constraint shapes this: the checksum's snapshot transaction pool **cannot grow** once the brief table lock is released, because every transaction must take its read view at the same instant. It is therefore provisioned up front at whatever ceiling scaling could reach — `vCPUs` with this flag, plain [threads](#threads) without it — reusing the read-side connection budget, since the copier's readers have finished by the time the checksum runs. An idle pooled transaction costs one connection and no extra history retention, so over-provisioning here is cheap; what it does cost is a slightly longer table lock while the snapshots are taken.
