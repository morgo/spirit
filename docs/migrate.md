...
| Field | Meaning |
| --- | --- |
| `%` | Rows settled out of the estimated total. The total comes from table statistics, so the percentage is approximate and not a row count you should reconcile against. Spirit re-reads those statistics every few minutes for the duration of the copy, so the total moves in both directions while the copy runs: on a table taking deletes the percentage can jump forward and then back across a refresh, and the settled count can overtake the estimate and read past 100%. Neither is a stalled or repeated copy, and the rows settled only ever increase. Rows the binlog applier wrote before the copy reached them are not counted (the copy inserts with `INSERT IGNORE`), so on a busy table the percentage finishes short of 100%. A resume from checkpoint continues the count from where the previous run left it. |
