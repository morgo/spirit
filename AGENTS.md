...
- **`pkg/checkpoint`** owns the one checkpoint-table schema and its create/drop/exists/write/read. Like sentinel, it keys on the connection's selected schema (`DATABASE()` / unqualified names), so callers point the `*sql.DB` at the right schema rather than passing one. `Write` always keeps a **single row** (`REPLACE` on `id=1`) — atomic, so a crash never leaves no checkpoint, and bounded, so datasync's continuous sync can't grow it. Two `Mode`s differ only in `Create`: `Owned` (DROP+CREATE: single-table and atomic multi-table migrations, move) vs `Persistent` (CREATE IF NOT EXISTS, never dropped/cleared: datasync's long-lived table, whose existence is its resume signal). Resume *policy* stays in each runner (migration: statement-match / collision / max-age; move: multi-source positions + check-framework gating; datasync: `Exists` vs "has a row"). The package interprets none of the watermarks. The multi-table `_spirit_checkpoint` is a fixed name shared by every multi-table migration in a schema, so it relies on the schema lock below to guarantee a single owner — that's what lets it be `Owned`/single-row rather than needing per-statement scoping.
