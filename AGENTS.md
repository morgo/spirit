...
- **`checksum.LocklessChecker`** is configured per-tool, and whether a confirmed stable divergence *aborts* or *self-heals* is an **explicit `CheckerConfig.DivergenceIsFatal` policy** (block/spirit#994) — it is not *inferred* from `Recopier` presence, and the two knobs are decoupled, so `DivergenceIsFatal: true` aborts **even if** a `Recopier` is supplied. **datasync**: `DivergenceIsFatal: false` plus a `MySQLRecopier`, set directly on the config it passes to `NewLocklessChecker` — it verifies a still-converging target, so divergences are expected and self-heal by recopying the chunk from source. **migration** goes through `NewChecker`, which does **not** take either field: it *derives* both from `CheckerConfig.FixDifferences`, which migration sets, so the lockless gate repairs a mismatched chunk exactly as the snapshot checker does and only gives up (with `ErrVerificationUnresolved`) once `MaxPasses` passes have failed to come back clean. A lockless migration therefore reaches `ErrPermanentDivergence` only if `FixDifferences` is off, which in the migration runner it is not. Both tools pace passes with `MinPassInterval`; migration's finite gate substitutes `RetryDelay` rather than `LocklessMinPassInterval`, because a cut-over is waiting on the answer.
