...
- **`pkg/checkpoint`** owns the one checkpoint-table schema and its create/drop/exists/write/read. Like sentinel, it keys on the connection's selected schema (`DATABASE()` / unqualified names), so callers point the `*sql.DB` at the right schema rather than passing one. `Write` always keeps a **single row** (`REPLACE` on `id=1`) — atomic, so a crash never leaves no checkpoint, and bounded, so datasync's continuous sync can't grow it. Two `Mode`s differ only in `Create`: `Transient` (DROP+CREATE — a checkpoint for one finite run: single-table and atomic multi-table migrations, move) vs `Persistent` (CREATE IF NOT EXISTS, never dropped/cleared — a checkpoint for a continuous run: datasync's long-lived table, whose existence is its resume signal). Resume *policy* stays in each runner (migration: statement-match / collision / max-age; move: multi-source positions + check-framework gating; datasync: `Exists` vs "has a row"). The package interprets none of the watermarks. The multi-table `_spirit_checkpoint` is a fixed name shared by every multi-table migration in a schema, so it relies on the schema lock below to guarantee a single owner — that's what lets it be `Transient`/single-row rather than needing per-statement scoping.
- **Recovery from an unreadable checkpoint** (e.g. one written by an incompatible version after a schema change) uses `checkpoint.IsIncompatible(err)` to tell a genuinely-unusable checkpoint apart from a transient read failure (permission, server gone) — so a blip never triggers a destructive recovery. migration falls back to a fresh migration automatically; move and datasync recover only under `--force`, which **wipes the target and starts fresh** (datasync drops the target database; move drops the target tables + checkpoint). Without `--force`, an unresumable non-empty target is a hard error that names `--force`.
- **Migration concurrency** — single-table migrations may run concurrently in a schema (serialized per-table by `dbconn.MetadataLock`'s `GET_LOCK`; two on the *same* table block each other). Atomic multi-table migrations additionally take a **schema-scoped** lock (`dbconn.WithMultiTableSchemaLock`, acquired in `Run` before any table is touched) so only one runs per schema at a time — they share the fixed-name `_spirit_checkpoint`/`_spirit_sentinel` and must not overlap. A second one fails fast. This is documented user-facing in README's "Atomic Multi-table changes".
