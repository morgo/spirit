...
| Lockless (optimistic) checksum | `pkg/checksum` `LocklessChecker` | migration (defer-cutover), datasync |
...
- **`checksum.LocklessChecker`** is configured per-tool, and whether a confirmed stable divergence *aborts* or *self-heals* is decided by whether the checker has a `Recopier` — there is no separate policy flag (the `DivergenceIsFatal` knob from block/spirit#994 is gone; it had one use site and was fully redundant with this). **migration** goes through `NewChecker`, which does not take a `Recopier` at all: it *derives* one from `CheckerConfig.FixDifferences`, which migration sets, so the lockless gate repairs a mismatched chunk exactly as the snapshot checker does and only gives up — with `ErrVerificationUnresolved` — once `MaxPasses` passes have failed to come back clean. A lockless migration therefore reaches `ErrPermanentDivergence` only if `FixDifferences` is off, which in the migration runner it is not. **datasync** passes its own `MySQLRecopier` to `NewLocklessChecker`, because it verifies across two servers and the factory can only build a repair path that writes back to the server it read from; its target is still converging, so divergences are expected and self-heal by recopying the chunk from source. Both tools pace passes with `MinPassInterval`; migration's finite gate substitutes `RetryDelay` rather than `LocklessMinPassInterval`, because a cut-over is waiting on the answer.
...
- **move's continuous checksum** still uses the distributed/sharded `checksum.Checker` (it is multi-source / possibly multi-target); `LocklessChecker` is single-source/single-target. Move is replication-backed like migration, so it would convert with the same derived-from-`FixDifferences` policy migration uses. The blocker is multi-source/multi-target support in `LocklessChecker`, not the abort policy.
- **Aurora autoscaling** is available in migration, move, and datasync behind `EnableExperimentalAutoscaling`. Datasync uses the shared copier controller during initial copy, the shared lockless-checksum limiter/controller afterwards, and a joined write controller for checksum repairs. Its target monitor stays open until `Close`; built-in feeds use `Runner.TargetUnderLoad` for adaptive flush narrowing, and injected feeds must wire that callback themselves. Bounds and monitor capacity are derived at startup (including resume); restart after changing target instance size. Do not confuse the applier's copy/repair workers with synchronous change-feed flush concurrency.
